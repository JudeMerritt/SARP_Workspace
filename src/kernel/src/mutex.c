...
#include "kernel/include/mutex.h" 
...
struct _int_mutex_t {
  ...
  uint32_t edit_lock;         // Edit lock for mutex (acquired when owner/lock_count are edited)
...
bool ti_acquire_mutex(const struct ti_mutex_t mutex, const int64_t timeout, enum ti_errc_t* const errc_out) {
  *errc_out = TI_ERRC_NONE;
  ...
  ti_atomic_store(&int_mutex->edit_lock, 0U);
  const int64_t start_time = ti_get_time(errc_out);
  if ( start_time == -1 ) { return false; }
  do {
    while (!ti_atomic_cmp_exchange((uint32_t*)&int_mutex->edit_lock, &(uint32_t){0U}, 1U));
    if (int_mutex->lock_count == 0) {
    ...
    ti_atomic_store(&int_mutex->edit_lock, 0U);
    ti_yield();
  } while ((ti_get_time(errc_out) - start_time) < timeout);
  return false;
}
